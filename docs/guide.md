...
- [五、配置手册](#五配置手册)
  ...
  - [5.5 memory — 记忆提取配置](#55-memory--记忆提取配置)
  - [5.6 compaction — 上下文压缩配置](#56-compaction--上下文压缩配置)
  - [5.7 chunking — 分块配置](#57-chunking--分块配置)
  - [5.8 search — 混合搜索配置](#58-search--混合搜索配置)
- [六、使用流程](#六使用流程)
  - [6.1 核心概念](#61-核心概念)
...
Replica 需要两个模型服务，均使用 OpenAI 兼容 API 协议。下面命令参考
`feat/qwen3.5-122b-a10b-fp8`、`feat/qwen3-embedding-4b`
两个分支中的部署参数整理而成。推荐显式设置 `--served-model-name`，这样客户端配置可以使用稳定的服务名，而不依赖本地权重目录名。
...
> `llm.model`、`embedding.model` 也应分别填写
> `Qwen3.5-122B-A10B-FP8`、`Qwen3-Embedding-4B`。
