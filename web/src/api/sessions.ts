...
export function extractMemory(sessionId: string) {
  return client.post<{ memory_count: number }>(`/v1/sessions/${sessionId}/extract-memory`, {}, { timeout: 300000 });
}

export function compactSession(sessionId: string) {
  return client.post<{ task_id: string; status: string; message: string }>(`/v1/sessions/${sessionId}/compact`);
}
...
export function getCompactionConfig() {
  return client.get<{ hard_threshold_tokens: number; keep_recent_tokens: number }>("/v1/config/compaction");
}
