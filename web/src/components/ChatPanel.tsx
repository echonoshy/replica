...
export default function ChatPanel() {
  ...
  const handleSend = async () => {
    ...
    await chatStream(
      ...
      useMemory,
      {
        ...
        onDone: async (messageId, _tokenCount) => {
          const aiMsg: Message = {
  ...
  const handleMemorize = async () => {
    ...
    try {
      ...
      setMemorizeResult(
        `提取失败: ${(e as { response?: { data?: { detail?: string } } }).response?.data?.detail ?? (e as Error).message}`,
      );
    } finally {
      setMemorizing(false);
    ...
    try {
      ...
      const pollInterval = setInterval(async () => {
        try {
          ...
          clearInterval(pollInterval);
          setCompactResult(`查询状态失败: ${(e as Error).message}`);
          setCompacting(false);
          setTimeout(() => setCompactResult(null), 5000);
        }
      ...
      setCompactResult(
        `启动压缩失败: ${(e as { response?: { data?: { detail?: string } } }).response?.data?.detail ?? (e as Error).message}`,
      );
      setCompacting(false);
      setTimeout(() => setCompactResult(null), 5000);
    }
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-6 text-foreground px-8 bg-background">
        ...
        <p className="text-sm text-center max-w-md font-bold bg-white p-4 border-2 border-border shadow-[4px_4px_0px_0px_#111111]">
          {currentUser ? "在左侧选择已有会话，或点击 + 创建新会话开始对话" : "请先在左侧选择用户，或创建新用户开始使用"}
        </p>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto px-6 py-6">
          {messages.length === 0 && !streamingText && (
            <div className="flex flex-col items-center justify-center h-full gap-6 text-foreground py-24">
              ...
              <p className="text-base font-bold uppercase tracking-widest bg-primary text-white px-4 py-2 border-2 border-border shadow-[4px_4px_0px_0px_#111111]">
                输入消息开始对话
              </p>
            </div>
          )}
        ...
        <div className="flex gap-2">
          {sessionActive && (
            <>
              ...
              >
                {compacting ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : <Trash2 className="h-4 w-4 mr-1" />}
                压缩会话
              </Button>
              <Button
                variant="default"
              ...
              >
                {memorizing ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : <Sparkles className="h-4 w-4 mr-1" />}
                提取记忆
              </Button>
            </>
          )}
